...
| IM-3 | Provenance must remain joinable across stages. | Schema: evidence item fields. Code: `provenance/retrieval_normalizer.py` normalization path. Tests: `tests/test_retrieval_provenance_normalizer.py` + strategy IM-3. |
| IM-4 | `doc_id` semantics must stay stable across ingestion, retrieval, and citations; `doc_id` must come from provenance metadata, not parsed from asset S3 key shape. | Schema: request/evidence/response schemas (`doc_id`, scoped fields). Code: `provenance/manifest_store.py` (DynamoDB-backed), `provenance/retrieval_normalizer.py`, ingestion registration script. Tests: `tests/test_ingestion_manifest_store.py`, `tests/test_retrieval_provenance_normalizer.py` + strategy IM-4. |
